...
"初爻は卦の最下位で、物事がまだ始まったばかりの段階を示す位置です。",
"二爻は下卦の中央で、内側の場で物事が形を取り始める位置です。",
"三爻は下卦の最上位で、内から外へ移る境目にあたる位置です。",
"四爻は上卦の最下位で、内側から外側・社会的な場へ踏み出した位置です。",
"五爻は上卦の中央で、伝統的には中心的・指導的な位置として重視されます。",
"上爻は卦の最上位で、物事が極まり、次の変化へ移ろうとする位置です。"
];
  ...
  const timer=setInterval(()=>{
    ...
    if(ticks>=10){
      ...
      $("trigramExplanation").textContent=
        "上卦の"+upper.name+"（"+upper.reading+"）は"+trigramMeaning[upper.name]+
        " 下卦の"+lower.name+"（"+lower.reading+"）は"+trigramMeaning[lower.name];
      $("lineExplanation").textContent=
        lineName+"は"+(bits[pos] ? "陽爻（九）" : "陰爻（六）")+"です。"+
        linePositionMeaning[pos]+
        " ここでは、この爻を問いにどう当てはめるかまでは判断しません。";
